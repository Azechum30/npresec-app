	...
	function SaveToDatabase() {
		startTransition(async () => {
			...
			if (response?.errors) {
				...
				toast.success(
					`${response?.count} records were uploaded successfully!`
				)
				onClose(dialogId)
				setData(undefined)
				setColumns([])
			...
			>
				<DialogHeader>
					...
					<DialogDescription className='italic underline'>
						Read the instructions carefully for a smooth data
						upload:
					</DialogDescription>
				</DialogHeader>
				<ul className='list-disc ml-4 text-sm text-muted-foreground '>
					<li className='leading-6'>
						The file format should be an excel file formatted and
						saved as a Comma Separated Version (CSV) file.
					</li>
					<li className='leading-6'>
						Ensure the column headers are properly named with the
						casing the server expects. If in doubt contact the
						System&apos;s Administrator.
					</li>
					<li className='leading-6'>
						To avoid confusion or unncessary errors, there is a
						template CSV file below which you can download and use
				...
				<CSVReader onUploadAccepted={onFileUploadAccepted}>
					{({
						...
						<>
							<div
								className={cn(
									"w-full max-w-md mx-auto rounded-2xl h-[200px] border border-dashed flex flex-col justify-center items-center gap-2 hover:cursor-pointer relative p-3",
