...
export const RenderAttendanceTable = ({promise}: RenderAttendanceTableProps) => {
	...
	const filterAttendance = useMemo(()=>{
		...
		return initialData.filter(attendance =>{
			...
			if(status && attendance.status.toString() !== status.trim()) return false;
	...
	const {isPending, handleMultipleDeleteAttendance} = useMultipleDeleteAttendance();

	if(attendancePromise?.error || classPromise?.error){
		toast.error(attendancePromise?.error || classPromise?.error);
		return  <div className="text-red-500">{attendancePromise?.error  || classPromise?.error}</div>
	}
	...
	return <>
		...
		<DataTable
			...
			filename={classId ? `Attendance for ${classPromise.data.filter(value => value.id === classId)[0].name}` : "Attendance"}
			onDelete={async(rows)=>{
