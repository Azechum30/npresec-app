...
import { AttendanceType, BulkAttendanceSchema, BulkAttendanceType } from "@/lib/validation";
...
import { getStudents } from "@/app/(private)/admin/students/actions/action";
...
export const CreateAttendanceForm:React.FC<CreateAttendanceFormProps> = ({onSubmit,defaultValues,id, isPending}) => {
	...
	const [classes, setClasses] = useState<Pick<ClassesResponseType, "id" | "name">[]>([]);
	const [students, setStudents] = useState<Pick<StudentResponseType, "id" | "firstName" | "lastName" | "studentNumber" | "classId">[]>([])
	...
	useEffect( () => {
		if(!selectClassId) return;
			const classStudents = students.filter(student=> student.classId === selectClassId);
	...
	const filterStudents = students.filter(student=> student.classId === selectClassId);

	return <Form {...form}>
		<form onSubmit={form.handleSubmit(handleSubmit)} className={cn("space-y-10 w-full text-left rounded-md border p-4", selectClassId && "h-full overflow-auto scrollbar-thin")}>
					...
					<Table>
						<TableHeader>
							<TableRow className="bg-orange-50 dark:bg-orange-900/20">
								<TableHead className="text-orange-800 dark:text-orange-200">Student ID</TableHead>
								<TableHead className="text-orange-800 dark:text-orange-200">First Name</TableHead>
								<TableHead className="text-orange-800 dark:text-orange-200">Last Name</TableHead>
								<TableHead className="text-orange-800 dark:text-orange-200">Attendance Status</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{fields.map((field, index)=>{
								const student = filterStudents.find(s => s.id === field.studentId);
								return (
									<TableRow key={field.id}>
										...
										<TableCell>
											<FormField control={form.control} name={`studentEntries.${index}.status`} render={({field})=>(
												<FormItem>
													<Select defaultValue={field.value} onValueChange={field.onChange}>
														<SelectTrigger className={
															cn(field.value === "Present" && "text-green-500", field.value === "Absent" && "text-red-500", field.value ==="Late" && "text-orange-500", field.value === "Excused" && "text-blue-500")
														}>
														...
														<FormControl>
															<SelectContent position="popper">
																...
																<SelectItem value="Late" className="text-orange-500">Late</SelectItem>
