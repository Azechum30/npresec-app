...
export const MarkSingleStudentAttendanceForm = ({isPending, id, defaultValues, onSubmitAction}:MarkSingleStudentAttendanceFormProps) => {
	...
	const [allStudents, setAllStudents] = useState<Pick<StudentResponseType, "id" | "firstName" | "lastName" | "classId">[]>([]);
	const [classes, setClasses] = useState<Pick<ClassesResponseType, "id" | "name">[]>([]);
	const [filteredStudents, setFilteredStudents] = useState<{id: string, fullName: string}[]>([]);
		...
		const fetchData = async () => {
			try {
				...
				if (classesPromise.data) {
					setClasses(id
						? classesPromise.data.filter(cls => cls.id === defaultValues?.classId)
						: classesPromise.data
					);
				}
				...
				if (studentsPromise.students) {
					const students = id
						? studentsPromise.students.filter(student => student.id === defaultValues?.studentId)
					...
					if (id && defaultValues?.studentId) {
						const student = studentsPromise.students.find(s => s.id === defaultValues.studentId);
						if (student) {
							setFilteredStudents([{
								id: student.id,
	...
	return (
		<Form {...form}>
			<form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 text-left border rounded-md p-4">
				<SelectWithLabel<SingleStudentAttendance>
					name="classId"
					fieldTitle="Class"
					data={classes}
					valueKey="id"
				...
				<DatePickerWithLabel<SingleStudentAttendance>
					name="date"
					fieldTitle="Date"
