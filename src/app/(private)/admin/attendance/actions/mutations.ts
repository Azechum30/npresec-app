...
export const createAttendance = async(values:unknown)=>{
	try {
		...
		if(!unValidData.success){
			const zodError = unValidData.error.errors.map(err => `${err.path[0]}, ${err.message}`).join(" \n")
		...
		if(err instanceof Prisma.PrismaClientKnownRequestError){
			if(err.code === "P2002"){
				...
				const errorMessage = targetFields.includes("classId")? "Attendance has already been recorded for this class on the said date!": "Attendance has already been recorded for this date!"
...
export const createSingleAttendance = async(values:unknown)=>{
	try {
		...
		if(attendanceExist){
			return {error: "Attendance has already been recorded for this student on the said date!"}
		}
		const academicYear = generateAcademicYear(date);
