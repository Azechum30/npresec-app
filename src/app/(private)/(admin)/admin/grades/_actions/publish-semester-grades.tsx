...
export const publishSemesterGrades = async (
  previousState: any,
  formData: FormData,
  ...
  try {
    ...
    const students = await prisma.student.findMany({
      where: { classId },
      select: { id: true }, // We only need IDs for the mapping
    });

    // 1. Generate current semester stats for ranking
    ...
    // 2. Perform the heavy lifting in a single transaction
    await prisma.$transaction(async (tx) => {
      // Step A: Upsert the current semester records for all students
      ...
      // / Step B: THE RIPPLE EFFECT
      for (const student of students) {
        // Fetch all published summaries for this student
        const allStudentSemesters = await tx.semesterSummary.findMany({
          where: { studentId: student.id, isPublished: true },
        });

        // Sort them chronologically: Year first, then use the mapping for Semester
